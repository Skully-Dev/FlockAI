...
public class WanderRandom : FlockBehavior //Get the methods and vars from FlockBehaviour, so it has to implement CalculateMove.
{
    ...
    private float sqrMinDistance = 1; //radius of 0.5 = 0.5 value, radius of 1 = 2 value
    ...
    {
        Vector2 wanderMove = Vector2.zero;

        if (agent.newWander)
        {
            agent.wanderPoint = Random.insideUnitCircle * circleSize; //get new wander position
            agent.newWander = false;
        }

        wanderMove = agent.wanderPoint - (Vector2)agent.transform.position; //direction to current wander target

        //distance between agent and current target
        if (wanderMove.sqrMagnitude < sqrMinDistance) //if close to wander position
        {
            agent.wanderPoint = Random.insideUnitCircle * circleSize; //get new wander position
            wanderMove = agent.wanderPoint - (Vector2)agent.transform.position; //get direction to position
        }
